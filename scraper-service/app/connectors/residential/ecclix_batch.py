...
Requires Doppler: ECCLIX_USERNAME, ECCLIX_PASSWORD, ECCLIX_COUNTIES
"""
...
# Legacy alias — prefer portal_bases_for() from ecclix_county_config
ECCLIX_URLS: dict[str, str] = {
    "scott": "https://www.ecclix.com",
    "clark": "https://www.ecclix.com",
    "madison": "https://www.ecclix.com",
    "bourbon": "https://www.ecclix.com",
    "woodford": "https://www.ecclix.com",
    "franklin": "https://www.ecclix.com",
}
...
class ECCLIXBatchConnector(BaseConnector):
    ...
    async def fetch(self, browser: Browser, params: dict[str, Any]) -> list[RawRecord]:
        ...
        if not username or not password:
            logger.warning("[ecclix] No credentials in Doppler")
            return []
        ...
        # Scheduler reads no_proxy before fetch; flag on connector for logging
        self._no_proxy = bool(params.get("no_proxy"))
    ...
    async def _resolve_portal_base(
        self, page, county: str, username: str, password: str
    ) -> str | None:
        ...
        for base in bases:
            try:
                ...
                if await portal.session_established(page):
                    ...
                    return base
                await portal.goto_index_search(page, base)
                if await portal.is_login_page(page):
                    continue
                if await portal.session_established(page) or await portal.verify_county_context(
                    page, county
                ):
    ...
    async def _process_instrument_row(
        ...
        prop_addr = extract_address_from_legal(legal)
    ...
    async def _load_top_owner_names(
        ...
        try:
            resp = (
                ...
                .select("owner_name, raw_payload")
                .in_("source_key", ["ecclix_csv_import", "ecclix_batch"])
            ...
            for row in resp.data or []:
                owner = (row.get("owner_name") or "").strip()
                if not is_human_owner(owner) or owner in names:
                    continue
                payload = row.get("raw_payload") or {}
                due = float(payload.get("amount_due") or 0)
                if due >= min_tax_due:
                    names.append(owner)
                if len(names) >= limit:
                    break
    ...
    async def _mode_address(
        ...
        if not addresses and params.get("use_pending_leads", True):
            addresses = await self._load_pending_addresses(
                params.get("limit", settings.ecclix_batch_threshold)
            )
        if not addresses:
            return []
        ...
        for county in counties:
            ...
            if not portal_url:
                continue
            try:
                for addr in addresses[: params.get("limit", 40)]:
                    await portal.search_by_address(page, addr)
                    rows = await portal.parse_result_rows(page, limit=15)
                    for row in rows:
                        rec = await self._process_instrument_row(
                            page, portal_url, county, row, download_documents
                        )
                        if rec:
                            rec.data["search_address"] = addr
                            records.append(rec)
                    await human_delay(2.0, 4.0)
            except Exception as exc:
                logger.error("[ecclix] address mode %s: %s", county, exc)
        return records
    ...
    def parse(self, raw: RawRecord) -> Lead:
        data = raw.data
        if data.get("search_module") == "delinquent_tax":
            return Lead(
                source_key=self.source_key,
                vertical=Vertical.RESIDENTIAL,
                jurisdiction=f"KY-{data.get('county', '').title()}",
                lead_type=LeadType.TAX_LIEN,
                owner_name=data.get("owner_name"),
                property_address=data.get("property_address"),
                parcel_number=data.get("parcel_number"),
                estimated_value=data.get("amount_due"),
                state="KY",
                raw_payload=data,
            )

        inst = (data.get("instrument_type") or "").upper()
        profile = data.get("search_profile") or ""
        if profile.startswith("securities") or "city_lien" in profile:
            data.setdefault("signal_channel", "city_lien")
        elif inst == "LP":
            data.setdefault("signal_channel", "lis_pendens")
        elif inst == "WILL" or "estate" in profile:
            data.setdefault("signal_channel", "probate")
        ...
        for needle, lt in DOC_TYPE_TO_LEAD:
            if needle in inst:
                lead_type = lt
                break
        if data.get("signal_channel") == "city_lien":
            lead_type = LeadType.CODE_VIOLATION
        ...
        case_id = f"{book}/{page_no}" if book or page_no else None
        ...
        return Lead(
            ...
            property_address=data.get("property_address") or extract_address_from_legal(
                data.get("legal_description") or ""
            ),
            case_id=case_id,
            case_filed_date=parse_recorded_date(data.get("recorded_date") or ""),
            estimated_value=data.get("consideration_amount"),
            state="KY",
            raw_payload={
                **data,
                "ecclix_enriched": True,
                "investment_scores": data.get("investment_scores"),
                ...
                "wholesaler_package": {
                    ...
                    "instrument_type": inst,
                    "document_path": data.get("storage_path"),
                    "document_downloaded": data.get("document_downloaded"),
                },
            },
        )
